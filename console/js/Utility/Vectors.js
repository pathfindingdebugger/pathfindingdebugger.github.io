...
const divideNumber = v => n => ({x:v.x/n, y:v.y/n, z:v.z/n});
...
const max = v1 => v2 => ({x:Math.max(v1.x,v2.x), y:Math.max(v1.y,v2.y)});
const min = v1 => v2 => ({x:Math.min(v1.x,v2.x), y:Math.min(v1.y,v2.y)});